...
template <typename T>
inline const T&
    Vector<T>::operator[](const unsigned int& i) const  // subscripting
{
    return v[i];
}
...
Vector<T>::extract(const std::set<unsigned int>& indexes) const {
    ...
    for (std::set<unsigned int>::const_iterator el = indexes.begin();
         el != indexes.end(); el++) {
        if (*el >= n)
            throw std::logic_error("Error extracting subvector: the indexes "
                                   "are out of vector bounds");
        tmp[i++] = v[*el];
    }
...
inline bool operator==(const Vector<T>& v, const Vector<T>& w) {
    if (v.size() != w.size())
        throw std::logic_error(
            "Vectors of different size are not confrontable");
...
inline std::ostream& operator<<(std::ostream& os, const Vector<T>& v) {
    ...
    os << std::setw(20) << std::setprecision(16) << v[v.size() - 1]
       << std::endl;
...
inline T dot_prod(const Vector<T>& a, const Vector<T>& b) {
    ...
    if (a.size() != b.size())
        throw std::logic_error(
            "Dotprod error: the vectors are not the same size");
...
public:
    ...
    inline Matrix<T>
                     extractColumns(const std::set<unsigned int>& indexes) const;
...
inline Vector<T> Matrix<T>::extractRow(const unsigned int i) const {
    if (i >= n)
        throw std::logic_error("Error in extractRow: trying to extract a row "
...
Matrix<T>::extractRows(const std::set<unsigned int>& indexes) const {
    ...
    for (std::set<unsigned int>::const_iterator el = indexes.begin();
        ...
        for (unsigned int j = 0; j < m; j++) {
            if (*el >= n)
                throw std::logic_error("Error extracting rows: the indexes are "
                                       "out of matrix bounds");
            tmp[i][j] = v[*el][j];
        }
        i++;
...
Matrix<T>::extractColumns(const std::set<unsigned int>& indexes) const {
    ...
    for (std::set<unsigned int>::const_iterator el = indexes.begin();
        ...
        for (unsigned int i = 0; i < n; i++) {
            if (*el >= m)
                throw std::logic_error("Error extracting columns: the indexes "
                                       "are out of matrix bounds");
            tmp[i][j] = v[i][*el];
        }
        j++;
    }
...
template <typename T>
inline Matrix<T>
Matrix<T>::extract(const std::set<unsigned int>& r_indexes,
                   const std::set<unsigned int>& c_indexes) const {
    ...
    for (std::set<unsigned int>::const_iterator r_el = r_indexes.begin();
        ...
        if (*r_el >= n)
            throw std::logic_error("Error extracting submatrix: the indexes "
                                   "are out of matrix bounds");
        j = 0;
...
inline void Matrix<T>::setRow(unsigned int i, const Matrix<T>& a) {
    ...
    if (a.nrows() != 1)
        throw std::logic_error(
            "Error setting matrix column with a non-row matrix");
    for (unsigned int j = 0; j < ncols(); j++)
        v[i][j] = a[0][j];
}
...
inline void Matrix<T>::setRows(const std::set<unsigned int>& indexes,
    ...
    for (std::set<unsigned int>::const_iterator el = indexes.begin();
        ...
        for (unsigned int j = 0; j < ncols(); j++) {
            if (*el >= n)
                throw std::logic_error("Error in setRows: trying to set a row "
                                       "out of matrix bounds");
            v[*el][j] = m[i][j];
        }
        i++;
...
inline void Matrix<T>::setColumns(const std::set<unsigned int>& indexes,
    ...
    for (std::set<unsigned int>::const_iterator el = indexes.begin();
        ...
        for (unsigned int i = 0; i < nrows(); i++) {
            if (*el >= m)
                throw std::logic_error("Error in setColumns: trying to set a "
                                       "column out of matrix bounds");
            v[i][*el] = a[i][j];
        }
        j++;
    }
...
inline void Matrix<T>::set(const std::set<unsigned int>& r_indexes,
    ...
    for (std::set<unsigned int>::const_iterator r_el = r_indexes.begin();
        ...
        for (std::set<unsigned int>::const_iterator c_el = c_indexes.begin();
             c_el != c_indexes.end(); c_el++) {
            if (*c_el >= m)
                throw std::logic_error("Error in set: trying to set a column "
                                       "out of matrix bounds");
            v[*r_el][*c_el] = a[i][j];
            j++;
        }
...
inline bool operator==(const Matrix<T>& a, const Matrix<T>& b) {
    if (a.nrows() != b.nrows() || a.ncols() != b.ncols())
        throw std::logic_error(
            "Matrices of different size are not confrontable");
...
T dist(const T& a, const T& b) {
    ...
    if (abs_a > abs_b)
        ...
        return (abs_b == ( T )0.0
                    ? ( T )0.0
                    : abs_b
                          * sqrt(( T )1.0 + (abs_a / abs_b) * (abs_a / abs_b)));
}
...
void svd(const Matrix<T>& A, Matrix<T>& U, Vector<T>& W, Matrix<T>& V) {
    ...
    for (k = n - 1; k >= 0; k--) {
        for (unsigned int its = 0; its < max_its; its++) {
            ...
            if (its == max_its)
                throw std::logic_error("Error svd: no convergence in the "
            ...
            g  = dist(f, ( T )1.0);
            f  = ((x - z) * (x + z) + h * ((y / (f + sign(f) * fabs(g))) - h))
                / x;
...
Matrix<T> pinv(const Matrix<T>& A) {
    ...
    for (unsigned int i = 0; i < A.nrows(); i++) {
        ...
        tmp.setColumn(
            i,
            dot_prod(dot_prod(dot_prod(V, Matrix<double>(DIAG, 1.0 / W, 0.0,
                                                         W.size(), W.size())),
                              t(U)),
                     e));
    }
...
int lu(const Matrix<T>& A, Matrix<T>& LU, Vector<unsigned int>& index) {
    if (A.ncols() != A.nrows())
        throw std::logic_error(
            "Error in LU decomposition: matrix must be squared");
...
Vector<T> lu_solve(const Matrix<T>& LU, const Vector<T>& b,
    ...
    if (LU.ncols() != LU.nrows())
        throw std::logic_error(
            "Error in LU solve: LU matrix should be squared");
    unsigned int n = LU.ncols();
    if (b.size() != n)
        throw std::logic_error("Error in LU solve: b vector must be of the "
                               "same dimensions of LU matrix");
    Vector<T> x(( T )0.0, n);
    int       i, j, p;
    T         sum;
...
T lu_det(const Matrix<T>& A) {
    if (A.ncols() != A.nrows())
        throw std::logic_error(
            "Error in LU determinant: matrix must be squared");
    unsigned int         d;
    Matrix<T>            LU;
    Vector<unsigned int> index;
...
void cholesky(const Matrix<T> A, Matrix<T>& LL) {
    ...
    for (unsigned int i = 0; i < n; i++) {
        for (unsigned int j = i; j < n; j++) {
            ...
            if (i == j) {
                if (sum <= 0.0)
                    throw std::logic_error("Error in Cholesky decomposition: "
                                           "matrix is not postive definite");
                LL[i][i] = sqrt(sum);
            }
            else
                LL[j][i] = sum / LL[i][i];
...
Vector<T> cholesky_solve(const Matrix<T>& LL, const Vector<T>& b) {
    if (LL.ncols() != LL.nrows())
        throw std::logic_error(
            "Error in Cholesky solve: matrix must be squared");
    unsigned int n = LL.ncols();
    if (b.size() != n)
        throw std::logic_error("Error in Cholesky decomposition: b vector must "
                               "be of the same dimensions of LU matrix");
    Vector<T> x, y;
...
void forward_elimination(const Matrix<T>& L, Vector<T>& y, const Vector<T> b) {
    ...
    if (b.size() != L.nrows())
        throw std::logic_error("Error in Forward elimination: b vector must be "
                               "of the same dimensions of L matrix");
...
void backward_elimination(const Matrix<T>& U, Vector<T>& x,
    ...
    if (y.size() != U.nrows())
        throw std::logic_error("Error in Backward elimination: b vector must "
                               "be of the same dimensions of U matrix");
    int n = y.size();
    x.resize(n);
...
Matrix<T> dot_prod(const Matrix<T>& a, const Vector<T>& b) {
    if (a.ncols() != b.size())
        throw std::logic_error("Error matrix dot product: dimensions of the "
                               "matrix and the vector are not compatible");
...
Matrix<T> dot_prod(const Vector<T>& a, const Matrix<T>& b) {
    if (a.size() != b.nrows())
        throw std::logic_error("Error matrix dot product: dimensions of the "
                               "vector and matrix are not compatible");
